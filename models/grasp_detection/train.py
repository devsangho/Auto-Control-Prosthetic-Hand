...
train_0RE = np.loadtxt('dataset/train_abs_{}_{}_6Feature_ch1_ch2_0RE_{}Hz.csv'.format(subject,date,samplingrate), np.float32,
                       delimiter=',')
train_1WF = np.loadtxt('dataset/train_abs_{}_{}_6Feature_ch1_ch2_1WF_{}Hz.csv'.format(subject,date,samplingrate), np.float32,
                       delimiter=',')
train_2WE = np.loadtxt('dataset/train_abs_{}_{}_6Feature_ch1_ch2_2WE_{}Hz.csv'.format(subject,date,samplingrate), np.float32,
                       delimiter=',')
# train_3FI = np.loadtxt('train_abs_{}_{}_6Feature_ch1_ch2_3FI_{}Hz.csv'.format(subject,date,samplingrate), np.float32,
#                        delimiter=',')

test_0RE = np.loadtxt('dataset/test_abs_{}_{}_6Feature_ch1_ch2_0RE_{}Hz.csv'.format(subject,date,samplingrate), np.float32,
                       delimiter=',')
test_1WF = np.loadtxt('dataset/test_abs_{}_{}_6Feature_ch1_ch2_1WF_{}Hz.csv'.format(subject,date,samplingrate), np.float32,
                       delimiter=',')
test_2WE = np.loadtxt('dataset/test_abs_{}_{}_6Feature_ch1_ch2_2WE_{}Hz.csv'.format(subject,date,samplingrate), np.float32,
                       delimiter=',')
...
svclassifier = OneVsRestClassifier(SVC(C=SVM_C_coefficient, kernel=kernel_type, gamma=gamma_val,probability=True))
...
if PRINT_CLFreport == 1:
    print("*****************************************************")
    print("Case :")
    print(classification_report(y_test, y_pred, target_names=['Rest', 'Flexion','Extension']))
